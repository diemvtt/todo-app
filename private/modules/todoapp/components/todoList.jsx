...
@connectToStore class TodoList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            filter: ALL
    ...
    render() {
        ...
        if (this.state.filter === ALL) {
            ...
            todoItems = this.props.todos.filter(todo => todo.status === false).map(todo => <TodoItem key={todo.id}
                                                                                                     todo={todo}/>);
        } else {
            todoItems = this.props.todos.filter(todo => todo.status === true).map(todo => <TodoItem key={todo.id}
                                                                                                    todo={todo}/>);
        }

        let edit = this.props.todos.length > 0 ?
            <input className="toggle-all" type="checkbox" checked={this.props.isCheckedAll} ref="checkall"
                   onChange={this.handleChange.bind(this)}/>
            : <div></div>

        let statistics = this.props.todos.length > 0 ?
            <div className="footer">
                ...
                <ul className="filter">
                    ...
                    <button onClick={this.handleSubmit.bind(this)} className="clear-completed">Clear completed
                    </button>
                </ul>
            </div>
            : <div></div>
